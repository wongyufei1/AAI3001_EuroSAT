...
def run(data_path):
    ...
    # save splits
    with open(os.path.join(save_dir, "rgb_multiclass_datasets_split.txt"), "w+") as file:
    ...
    for t_label, t_model in transform_best_models.items():
        ...
        mean_avg_precision, classes_avg_precision = euro_sat_rgb_evaluator.avg_precision_by_class(probs.to("cpu"),
                                                                                                  labels)
    ...
    # save weights
    torch.save(best_model["weights"], os.path.join(save_dir, "rgb_multiclass_model.pt"))

    with open(os.path.join(save_dir, "rgb_multiclass_params.txt"), "w+") as file:
        file.write("transform,parameter,epoch\n")
        file.write(",".join([best_model["transform"], str(best_model["param"]), str(best_model["epoch"])]))

    with open(os.path.join(save_dir, "rgb_multiclass_train_losses.txt"), "w+") as file:
        file.write(",".join(map(str, best_model["model"].train_losses)))

    with open(os.path.join(save_dir, "rgb_multiclass_valid_losses.txt"), "w+") as file:
