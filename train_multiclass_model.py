...
def train_model():
    ...
    for t_label, t_model in transform_best_models.items():
        ...
        mean_accuracy, classes_accuracy = euro_sat_rgb_evaluator.accuracy_by_class(
            one_hot_preds, labels)

        print(f"\n{'Class':<25} {'Average Precision':<25} {'Accuracy':<15}\n")
        for idx, avg_precision in enumerate(classes_avg_precision):
            print(
                f"{indices_to_labels[idx]:<25} {avg_precision:<25} {classes_accuracy[idx]:<15}")
    ...
    with open("rgb_multiclass_params.txt", "w+") as file:
        file.write(",".join([best_model["transform"], str(
            best_model["param"]), str(best_model["epoch"])]))
