...
def train_model():
    ...
    # save splits
    # save splits
    with open("rgb_multilabel_datasets_split.txt", "w+") as file:
    ...
    dataloaders = {
        ...
        "valid": torch.utils.data.DataLoader(datasets["valid"], batch_size=batch_size, shuffle=True)
    }
    ...
    # save best model
    torch.save(best_model["weights"], "rgb_multilabel_model.pt")

    with open("rgb_multilabel_params.txt", "w+") as file:
        file.write("transform,parameter,epoch\n")
        file.write(",".join(["flip", str(best_model["param"]), str(best_model["epoch"])]))

    # save losses
    with open("rgb_multilabel_train_losses.txt", "w+") as file:
        file.write(",".join(map(str, best_model["model"].train_losses)))

    with open("rgb_multilabel_valid_losses.txt", "w+") as file:
